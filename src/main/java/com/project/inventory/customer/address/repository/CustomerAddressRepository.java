...
public interface CustomerAddressRepository extends JpaRepository<CustomerAddress, Integer> {
    ...
    @Query(value = "SELECT * FROM customer_addresses WHERE account_id =:id and is_deleted = 0", nativeQuery = true)
    List<CustomerAddress> findAllCustomerAddresses( @Param ( "id" ) int id);

    Optional<CustomerAddress> findById(int id);

    @Query(value = "SELECT * FROM customer_addresses WHERE is_default = 1 AND account_id = :accountId and is_deleted = 0", nativeQuery = true)
